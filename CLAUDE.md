...
- **Manga gameplay standard**: gameplay manga mode should use one coherent set per game: a gameplay background, animated character/enemy sprite sheets with `meta.animations`, and an FX/HUD sheet registered in that game's manifest. Add a per-game manga stage model for horizon/depth/projection so sprites inherit scene scale, shadows, lighting, and occlusion. `GodGames.MangaArt.drawAnimation(...)` is the shared project helper for timing sheet frames. Avoid mixing an asset-backed manga character over an unrelated procedural battlefield unless the asset is still loading.
...
- **Storage keys** in `localStorage`: `godgames_playerName` (string), `godgames_manga` (`'1'` or absent), `godgames_perf` (`'high' | 'balanced' | 'low'`; manga runtime defaults to balanced), `icarus_lastPlay` (timestamp; controls tutorial skip).
