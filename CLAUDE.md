...
- **Manga gameplay standard**: gameplay manga mode should use one coherent set per game: a gameplay background, animated character/enemy sprite sheets with `meta.animations`, and an FX/HUD sheet registered in that game's manifest. `GodGames.MangaArt.drawAnimation(...)` is the shared project helper for timing sheet frames. Avoid mixing an asset-backed manga character over an unrelated procedural battlefield unless the asset is still loading.
