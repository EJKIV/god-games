...
- **Manga gameplay standard**: gameplay manga mode should use one coherent set per game: a gameplay background, animated character/enemy sprite sheets with `meta.animations`, and an FX/HUD sheet registered in that game's manifest. Add a per-game manga stage model for horizon/depth/projection so sprites inherit scene scale, shadows, lighting, and occlusion. `GodGames.MangaArt.drawAnimation(...)` is the shared project helper for raw sheet timing; use `drawActedAnimation(...)` when gameplay stride/flap cycles should drive frames via `phaseTime` while still getting film-style tweening. Avoid mixing an asset-backed manga character over an unrelated procedural battlefield unless the asset is still loading.
- **Mystery scene-cuts**: game secrets call `GodGames.Mysteries.unlockAndDepart({ hintId, placeId, fromGame })`, which earns the hint, reveals that hint's canonical clue, and navigates to `place.html`. The manga-codeword chain is fixed easiest-to-hardest (`toad -> Icarus -> Orion -> Achilles -> Perseus -> Olympus`) while allowing out-of-order discoveries; clue state stays in `Engine.unlock` so `progress-sync.js` keeps syncing it. The ZEUS chain is storage-versioned (`hint.v3.*`, `clue.v3.*`, `manga_mode.v3`) so old clue discoveries can be globally reset without deleting unrelated progress. `places.js` is the data-driven catalog consumed by that page; see its top-of-file docs before adding locations.
...
| 2026-05-11 | Reworked the ZEUS easter egg into a toad-journal myth puzzle with canonical clue storage, realm-specific uncovering text, and out-of-order discovery handling. | codex  |
...
| 2026-05-10 | Refined the manga hub so destination murals sit as wall paintings inside the Olympus shrine bays, labels read as etched stone, and the hub traveler uses an asset-backed manga avatar sheet. | codex  |
